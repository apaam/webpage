...
The script:
    1. Collects all .md files under docs/python-api/ in sidebar order
    2. Extracts title from the first file's frontmatter (fallback to default)
    3. Merges them in the same order as the autogenerated python-api sidebar
...
def main() -> None:
    ...
    files = pdf_doc_order.ordered_paths_for_python_api_pdf(project_root)
    if not files:
