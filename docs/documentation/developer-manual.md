...
- We use [doxygen](https://www.doxygen.nl/index.html) to generate the code documentation. We suggest the following comment format.
    ...
    - Other commands if appropriate: `@warning {warning message}`, `@todo {things to be done}`, `@bug`, `@brief`, `@var`, `@enum`, `@struct`, `@class`.
...
- To probe the performance：
    ...
    Example performance counter stats:
...
- To generate heat map and visual:
        ...
        # then upload out.perf to https://www.speedscope.app/ to visual