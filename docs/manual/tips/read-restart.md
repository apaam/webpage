---
title: "Read Restart Usage"
---

## Read Restart Usage

Phynexis supports two restart mechanisms to facilitate flexible and efficient continuation of large-scale discrete element simulations. Users can choose between **VTK-based** and **JSON-based** restart options depending on the fidelity required and the intended use (e.g., visualization vs. simulation continuation).

The minimal Python pattern matches [Basic usage](../basic-usage.md): write checkpoints under your `DataDumper` root (e.g. `tmp/out/`), then load with `AutoReadRestart` as shown below.
...
Where:
- `"tmp/out/"` is the directory containing the saved restart data (same convention as [Basic usage](../basic-usage.md); use any path you configured with `SetRootPath`).
