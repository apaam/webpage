...
| 2026-05-03 | fields | fix | `pycsr_matrix.cpp`: missing `#include "utils/types/pyvecx_type_caster.hpp"` caused `VecX<double>` / `VecX<Int64>` conversion failures in `get_row()`, `get_row_indices()`, `multiply(vector)`, `set_elements()`, `add_elements()`. Added include. | fixed |
